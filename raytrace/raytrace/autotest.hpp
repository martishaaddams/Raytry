...
void autotest2() {
    ...
    std::vector<box*> spheres = scan_box("spheres_data2.txt", scr_data);
    if (spheres.size() == 0) {
        return;
    }
    render(scr_data, spheres);
...
void autotest3() {
    ...
    std::vector<box*> spheres = scan_box("spheres_data3.txt", scr_data);
    if (spheres.size() == 0) {
        return;
    }
    ...
    std::cout<<T<<std::endl;
   /* for (std::vector<box*>::iterator it = spheres.begin(); it != spheres.end(); ++it) {
